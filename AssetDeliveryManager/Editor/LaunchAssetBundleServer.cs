...
namespace AssetDelivery
{
    internal class LaunchAssetBundleServer : ScriptableSingleton<LaunchAssetBundleServer>
    {
        ...
        static void Run()
        {
            string pathToAssetServer = Path.GetFullPath("Assets/Scripts/Common/AssetBundleManager/Editor/AssetBundleServer.exe");
        ...
        static string GetMonoProfileVersion()
        {
            ...
            for (int i = 0; i < foldersWithApi.Length; i++)
            {
                ...
                Version version = new Version(foldersWithApi[i]);
                if (version.CompareTo(profileVersion) > 0)
                {
